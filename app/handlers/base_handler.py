...
class BaseHandler(http.server.BaseHTTPRequestHandler, ABC):
    ...
    def send_json_response(self, data: Dict[str, Any], status_code: int = HTTP_OK):
        """Send JSON response."""
    ...
    def send_html_response(self, html: str, status_code: int = HTTP_OK):
        """Send HTML response."""
        self.send_response(status_code)
